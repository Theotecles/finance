...
query = '''
    ...
    WHERE symbol = 'AEE'
    AND data_date >= '2009-06-01'
    ORDER BY data_date ASC
    '''

# PULL IN THE DATA USING PANDAS
aee_df = pd.read_sql(query, conn)
print(aee_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
aee_dates = []
for date in aee_df['data_date']:
    aee_dates.append(datetime.strptime(date, '%Y-%m-%d'))

aee_df['data_date'] = aee_dates
symbol = aee_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(aee_dates, aee_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(aee_df['adjusted_close']), max(aee_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
aee_df = aee_df.drop(['symbol'], axis=1)
aee_df['data_date'] = pd.to_datetime(aee_df['data_date'])
aee_df.set_index('data_date', inplace=True)
aee_df.index = pd.DatetimeIndex(aee_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = aee_df.head(round(len(aee_df)*.75))
test = aee_df.tail(round(len(aee_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2018, 8, 24)
...
# RMSE for Model (10, 2, 2): 14.10454657045353
# FIT THE MODEL
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(aee_df['adjusted_close']), max(test['predictions']))

plt.show()

# FIT AND PLOT FINAL MODEL
model = ARIMA(aee_df, order=(10, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(aee_df['adjusted_close']), max(projections))
