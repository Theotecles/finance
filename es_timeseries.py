...
es_df = pd.read_sql(query, conn)
print(es_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
es_dates = []
for date in es_df['data_date']:
    es_dates.append(datetime.strptime(date, '%Y-%m-%d'))

es_df['data_date'] = es_dates
symbol = es_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(es_dates, es_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(es_df['adjusted_close']), max(es_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
es_df = es_df.drop(['symbol'], axis=1)
es_df['data_date'] = pd.to_datetime(es_df['data_date'])
es_df.set_index('data_date', inplace=True)
es_df.index = pd.DatetimeIndex(es_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = es_df.head(round(len(es_df)*.75))
test = es_df.tail(round(len(es_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 2, 1): 3.3692979344910325
# FIT THE MODEL
...
model = ARIMA(es_df, order=(10, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
