...
aap_df = pd.read_sql(query, conn)
print(aap_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
aap_dates = []
for date in aap_df['data_date']:
    aap_dates.append(datetime.strptime(date, '%Y-%m-%d'))

aap_df['data_date'] = aap_dates

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(aap_dates, aap_df['adjusted_close'], c='red', alpha=0.6)

# FORMAT PLOT
ax.set_title(f"Daily Adjusted Close Data \n({aap_df['symbol'][1]})", fontsize=24)
ax.set_xlabel('', fontsize=16)
fig.autofmt_xdate()
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(aap_df['adjusted_close']), max(aap_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
aap_df = aap_df.drop(['symbol'], axis=1)
aap_df['data_date'] = pd.to_datetime(aap_df['data_date'])
aap_df.set_index('data_date', inplace=True)
aap_df.index = pd.DatetimeIndex(aap_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = aap_df.head(round(len(aap_df)*.75))
test = aap_df.tail(round(len(aap_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 0, 0): 38.452858059168236
# FIT THE MODEL
...
model = ARIMA(aap_df, order=(0, 0, 0))
model_fit = model.fit()
print(model_fit.summary())
