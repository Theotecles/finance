...
aapl_df = pd.read_sql(query, conn)
print(aapl_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
aapl_dates = []
for date in aapl_df['data_date']:
    aapl_dates.append(datetime.strptime(date, '%Y-%m-%d'))

aapl_df['data_date'] = aapl_dates
symbol = aapl_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(aapl_dates, aapl_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(aapl_df['adjusted_close']), max(aapl_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
aapl_df = aapl_df.drop(['symbol'], axis=1)
aapl_df['data_date'] = pd.to_datetime(aapl_df['data_date'])
aapl_df.set_index('data_date', inplace=True)
aapl_df.index = pd.DatetimeIndex(aapl_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = aapl_df.head(round(len(aapl_df)*.75))
test = aapl_df.tail(round(len(aapl_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (8, 1, 2): 10.836134838738838
# FIT THE MODEL
...
model = ARIMA(aapl_df, order=(8, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
