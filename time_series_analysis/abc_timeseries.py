...
abc_df = pd.read_sql(query, conn)
print(abc_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
abc_dates = []
for date in abc_df['data_date']:
    abc_dates.append(datetime.strptime(date, '%Y-%m-%d'))

abc_df['data_date'] = abc_dates
symbol = abc_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(abc_dates, abc_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(abc_df['adjusted_close']), max(abc_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
abc_df = abc_df.drop(['symbol'], axis=1)
abc_df['data_date'] = pd.to_datetime(abc_df['data_date'])
abc_df.set_index('data_date', inplace=True)
abc_df.index = pd.DatetimeIndex(abc_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = abc_df.head(round(len(abc_df)*.75))
test = abc_df.tail(round(len(abc_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (2, 1, 1): 24.417596771963364
# FIT THE MODEL
...
model = ARIMA(abc_df, order=(2, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
