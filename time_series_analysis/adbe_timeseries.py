...
adbe_df = pd.read_sql(query, conn)
print(adbe_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
adbe_dates = []
for date in adbe_df['data_date']:
    adbe_dates.append(datetime.strptime(date, '%Y-%m-%d'))

adbe_df['data_date'] = adbe_dates

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(adbe_dates, adbe_df['adjusted_close'], c='red', alpha=0.6)

# FORMAT PLOT
ax.set_title(f"Daily Adjusted Close Data \n({adbe_df['symbol'][1]})", fontsize=24)
ax.set_xlabel('', fontsize=16)
fig.autofmt_xdate()
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(adbe_df['adjusted_close']), max(adbe_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
adbe_df = adbe_df.drop(['symbol'], axis=1)
adbe_df['data_date'] = pd.to_datetime(adbe_df['data_date'])
adbe_df.set_index('data_date', inplace=True)
adbe_df.index = pd.DatetimeIndex(adbe_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = adbe_df.head(round(len(adbe_df)*.75))
test = adbe_df.tail(round(len(adbe_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 1): 102.83080197367751
# FIT THE MODEL
...
model = ARIMA(adbe_df, order=(6, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
