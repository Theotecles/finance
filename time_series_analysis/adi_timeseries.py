...
adi_df = pd.read_sql(query, conn)
print(adi_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
adi_dates = []
for date in adi_df['data_date']:
    adi_dates.append(datetime.strptime(date, '%Y-%m-%d'))

adi_df['data_date'] = adi_dates
symbol = adi_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(adi_dates, adi_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(adi_df['adjusted_close']), max(adi_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
adi_df = adi_df.drop(['symbol'], axis=1)
adi_df['data_date'] = pd.to_datetime(adi_df['data_date'])
adi_df.set_index('data_date', inplace=True)
adi_df.index = pd.DatetimeIndex(adi_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = adi_df.head(round(len(adi_df)*.75))
test = adi_df.tail(round(len(adi_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2020, 4, 17)
...
# RMSE for Model (10, 2, 1): 16.221915003247837
# FIT THE MODEL
...
model = ARIMA(adi_df, order=(10, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(adi_df['adjusted_close']), max(projections))
