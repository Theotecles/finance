...
adm_dates = []
for date in adm_df['data_date']:
    adm_dates.append(datetime.strptime(date, '%Y-%m-%d'))

adm_df['data_date'] = adm_dates
symbol = adm_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(adm_dates, adm_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(adm_df['adjusted_close']), max(adm_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
adm_df = adm_df.drop(['symbol'], axis=1)
adm_df['data_date'] = pd.to_datetime(adm_df['data_date'])
adm_df.set_index('data_date', inplace=True)
adm_df.index = pd.DatetimeIndex(adm_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = adm_df.head(round(len(adm_df)*.75))
test = adm_df.tail(round(len(adm_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 2, 2): 8.331601018507934
# FIT THE MODEL
...
model = ARIMA(adm_df, order=(6, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
