...
adsk_df = pd.read_sql(query, conn)
print(adsk_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
adsk_dates = []
for date in adsk_df['data_date']:
    adsk_dates.append(datetime.strptime(date, '%Y-%m-%d'))

adsk_df['data_date'] = adsk_dates
symbol = adsk_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(adsk_dates, adsk_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(adsk_df['adjusted_close']), max(adsk_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
adsk_df = adsk_df.drop(['symbol'], axis=1)
adsk_df['data_date'] = pd.to_datetime(adsk_df['data_date'])
adsk_df.set_index('data_date', inplace=True)
adsk_df.index = pd.DatetimeIndex(adsk_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = adsk_df.head(round(len(adsk_df)*.75))
test = adsk_df.tail(round(len(adsk_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2020, 4, 29)
...
# RMSE for Model (6, 2, 1): 24.915046916613363
# FIT THE MODEL
...
model = ARIMA(adsk_df, order=(6, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
