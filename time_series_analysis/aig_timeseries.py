...
aig_df = pd.read_sql(query, conn)
print(aig_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
aig_dates = []
for date in aig_df['data_date']:
    aig_dates.append(datetime.strptime(date, '%Y-%m-%d'))

aig_df['data_date'] = aig_dates
symbol = aig_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(aig_dates, aig_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(aig_df['adjusted_close']), max(aig_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
aig_df = aig_df.drop(['symbol'], axis=1)
aig_df['data_date'] = pd.to_datetime(aig_df['data_date'])
aig_df.set_index('data_date', inplace=True)
aig_df.index = pd.DatetimeIndex(aig_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = aig_df.head(round(len(aig_df)*.75))
test = aig_df.tail(round(len(aig_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 0, 0): 9.687033085697145
# FIT THE MODEL
...
model = ARIMA(aig_df, order=(0, 0, 0))
model_fit = model.fit()
print(model_fit.summary())
