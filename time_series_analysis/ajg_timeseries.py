...
ajg_df = pd.read_sql(query, conn)
print(ajg_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
ajg_dates = []
for date in ajg_df['data_date']:
    ajg_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ajg_df['data_date'] = ajg_dates
symbol = ajg_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ajg_dates, ajg_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ajg_df['adjusted_close']), max(ajg_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ajg_df = ajg_df.drop(['symbol'], axis=1)
ajg_df['data_date'] = pd.to_datetime(ajg_df['data_date'])
ajg_df.set_index('data_date', inplace=True)
ajg_df.index = pd.DatetimeIndex(ajg_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ajg_df.head(round(len(ajg_df)*.75))
test = ajg_df.tail(round(len(ajg_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (8, 2, 1): 19.905698207625647
# FIT THE MODEL
...
model = ARIMA(ajg_df, order=(8, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
