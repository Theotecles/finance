...
all_dates = []
for date in all_df['data_date']:
    all_dates.append(datetime.strptime(date, '%Y-%m-%d'))

all_df['data_date'] = all_dates

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(all_dates, all_df['adjusted_close'], c='red', alpha=0.6)

# FORMAT PLOT
ax.set_title(f"Daily Adjusted Close Data \n({all_df['symbol'][1]})", fontsize=24)
ax.set_xlabel('', fontsize=16)
fig.autofmt_xdate()
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(all_df['adjusted_close']), max(all_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
all_df = all_df.drop(['symbol'], axis=1)
all_df['data_date'] = pd.to_datetime(all_df['data_date'])
all_df.set_index('data_date', inplace=True)
all_df.index = pd.DatetimeIndex(all_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = all_df.head(round(len(all_df)*.75))
test = all_df.tail(round(len(all_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 4, 16)
...
# RMSE for Model (10, 2, 1): 3.72848796008204
# FIT THE MODEL
...
model = ARIMA(all_df, order=(10, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(all_df['adjusted_close']), max(predictions))
