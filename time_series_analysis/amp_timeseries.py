...
amp_df = pd.read_sql(query, conn)
print(amp_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
amp_dates = []
for date in amp_df['data_date']:
    amp_dates.append(datetime.strptime(date, '%Y-%m-%d'))

amp_df['data_date'] = amp_dates
symbol = amp_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(amp_dates, amp_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(amp_df['adjusted_close']), max(amp_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
amp_df = amp_df.drop(['symbol'], axis=1)
amp_df['data_date'] = pd.to_datetime(amp_df['data_date'])
amp_df.set_index('data_date', inplace=True)
amp_df.index = pd.DatetimeIndex(amp_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = amp_df.head(round(len(amp_df)*.75))
test = amp_df.tail(round(len(amp_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (4, 1, 0): 11.674622227374599
# FIT THE MODEL
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(amp_df['adjusted_close']), max(test['predictions']))

plt.show()

# FIT AND PLOT FINAL MODEL
model = ARIMA(amp_df, order=(4, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(amp_df['adjusted_close']), max(projections))
