...
anet_df = pd.read_sql(query, conn)
print(anet_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
anet_dates = []
for date in anet_df['data_date']:
    anet_dates.append(datetime.strptime(date, '%Y-%m-%d'))

anet_df['data_date'] = anet_dates
symbol = anet_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(anet_dates, anet_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(anet_df['adjusted_close']), max(anet_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
anet_df = anet_df.drop(['symbol'], axis=1)
anet_df['data_date'] = pd.to_datetime(anet_df['data_date'])
anet_df.set_index('data_date', inplace=True)
anet_df.index = pd.DatetimeIndex(anet_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = anet_df.head(round(len(anet_df)*.75))
test = anet_df.tail(round(len(anet_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (2, 1, 2): 41.108129195286295
# FIT THE MODEL
...
model = ARIMA(anet_df, order=(2, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
