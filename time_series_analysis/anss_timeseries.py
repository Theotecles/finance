...
anss_df = pd.read_sql(query, conn)
print(anss_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
anss_dates = []
for date in anss_df['data_date']:
    anss_dates.append(datetime.strptime(date, '%Y-%m-%d'))

anss_df['data_date'] = anss_dates
symbol = anss_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(anss_dates, anss_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(anss_df['adjusted_close']), max(anss_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
anss_df = anss_df.drop(['symbol'], axis=1)
anss_df['data_date'] = pd.to_datetime(anss_df['data_date'])
anss_df.set_index('data_date', inplace=True)
anss_df.index = pd.DatetimeIndex(anss_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = anss_df.head(round(len(anss_df)*.75))
test = anss_df.tail(round(len(anss_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2020, 10, 20)
...
# RMSE for Model (10, 1, 0): 21.696349065641144
# FIT THE MODEL
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(anss_df['adjusted_close']), max(test['predictions']))

plt.show()

# FIT AND PLOT FINAL MODEL
model = ARIMA(anss_df, order=(10, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
