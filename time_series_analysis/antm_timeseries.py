...
antm_df = pd.read_sql(query, conn)
print(antm_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
antm_dates = []
for date in antm_df['data_date']:
    antm_dates.append(datetime.strptime(date, '%Y-%m-%d'))

antm_df['data_date'] = antm_dates
symbol = antm_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(antm_dates, antm_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(antm_df['adjusted_close']), max(antm_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
antm_df = antm_df.drop(['symbol'], axis=1)
antm_df['data_date'] = pd.to_datetime(antm_df['data_date'])
antm_df.set_index('data_date', inplace=True)
antm_df.index = pd.DatetimeIndex(antm_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = antm_df.head(round(len(antm_df)*.75))
test = antm_df.tail(round(len(antm_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 4, 22)
...
# RMSE for Model (1, 1, 0): 8.225472183863753
# FIT THE MODEL
...
model = ARIMA(antm_df, order=(1, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(antm_df['adjusted_close']), max(projections))
