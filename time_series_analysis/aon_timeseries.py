...
aon_df = pd.read_sql(query, conn)
print(aon_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
aon_dates = []
for date in aon_df['data_date']:
    aon_dates.append(datetime.strptime(date, '%Y-%m-%d'))

aon_df['data_date'] = aon_dates
symbol = aon_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(aon_dates, aon_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(aon_df['adjusted_close']), max(aon_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
aon_df = aon_df.drop(['symbol'], axis=1)
aon_df['data_date'] = pd.to_datetime(aon_df['data_date'])
aon_df.set_index('data_date', inplace=True)
aon_df.index = pd.DatetimeIndex(aon_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = aon_df.head(round(len(aon_df)*.75))
test = aon_df.tail(round(len(aon_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2020, 4, 22)
...
# RMSE for Model (10, 2, 2): 21.476421900523313
# FIT THE MODEL
...
model = ARIMA(aon_df, order=(10, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(aon_df['adjusted_close']), max(projections))
