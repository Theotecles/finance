...
aos_df = pd.read_sql(query, conn)
print(aos_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
aos_dates = []
for date in aos_df['data_date']:
    aos_dates.append(datetime.strptime(date, '%Y-%m-%d'))

aos_df['data_date'] = aos_dates
symbol = aos_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(aos_dates, aos_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(aos_df['adjusted_close']), max(aos_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
aos_df = aos_df.drop(['symbol'], axis=1)
aos_df['data_date'] = pd.to_datetime(aos_df['data_date'])
aos_df.set_index('data_date', inplace=True)
aos_df.index = pd.DatetimeIndex(aos_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = aos_df.head(round(len(aos_df)*.75))
test = aos_df.tail(round(len(aos_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 2, 0): 3.196199456715984
# FIT THE MODEL
...
model = ARIMA(aos_df, order=(10, 2, 0))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(aos_df['adjusted_close']), max(projections))
