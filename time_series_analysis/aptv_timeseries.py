...
aptv_df = pd.read_sql(query, conn)
print(aptv_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
aptv_dates = []
for date in aptv_df['data_date']:
    aptv_dates.append(datetime.strptime(date, '%Y-%m-%d'))

aptv_df['data_date'] = aptv_dates
symbol = aptv_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(aptv_dates, aptv_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(aptv_df['adjusted_close']), max(aptv_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
aptv_df = aptv_df.drop(['symbol'], axis=1)
aptv_df['data_date'] = pd.to_datetime(aptv_df['data_date'])
aptv_df.set_index('data_date', inplace=True)
aptv_df.index = pd.DatetimeIndex(aptv_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = aptv_df.head(round(len(aptv_df)*.75))
test = aptv_df.tail(round(len(aptv_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 1, 2): 8.625568076329102
# FIT THE MODEL
...
model = ARIMA(aptv_df, order=(10, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
