...
are_df = pd.read_sql(query, conn)
print(are_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
are_dates = []
for date in are_df['data_date']:
    are_dates.append(datetime.strptime(date, '%Y-%m-%d'))

are_df['data_date'] = are_dates

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(are_dates, are_df['adjusted_close'], c='red', alpha=0.6)

# FORMAT PLOT
ax.set_title(f"Daily Adjusted Close Data \n({are_df['symbol'][1]})", fontsize=24)
ax.set_xlabel('', fontsize=16)
fig.autofmt_xdate()
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(are_df['adjusted_close']), max(are_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
are_df = are_df.drop(['symbol'], axis=1)
are_df['data_date'] = pd.to_datetime(are_df['data_date'])
are_df.set_index('data_date', inplace=True)
are_df.index = pd.DatetimeIndex(are_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = are_df.head(round(len(are_df)*.75))
test = are_df.tail(round(len(are_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (2, 2, 1): 15.945976423889917
# FIT THE MODEL
...
model = ARIMA(are_df, order=(2, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(are_df['adjusted_close']), max(projections))
