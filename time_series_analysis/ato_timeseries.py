...
ato_df = pd.read_sql(query, conn)
print(ato_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
ato_dates = []
for date in ato_df['data_date']:
    ato_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ato_df['data_date'] = ato_dates
symbol = ato_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ato_dates, ato_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ato_df['adjusted_close']), max(ato_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ato_df = ato_df.drop(['symbol'], axis=1)
ato_df['data_date'] = pd.to_datetime(ato_df['data_date'])
ato_df.set_index('data_date', inplace=True)
ato_df.index = pd.DatetimeIndex(ato_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ato_df.head(round(len(ato_df)*.75))
test = ato_df.tail(round(len(ato_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (8, 2, 2): 3.427759956447834
# FIT THE MODEL
...
model = ARIMA(ato_df, order=(8, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
