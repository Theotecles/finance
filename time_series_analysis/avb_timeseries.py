...
avb_df = pd.read_sql(query, conn)
print(avb_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
avb_dates = []
for date in avb_df['data_date']:
    avb_dates.append(datetime.strptime(date, '%Y-%m-%d'))

avb_df['data_date'] = avb_dates
symbol = avb_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(avb_dates, avb_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(avb_df['adjusted_close']), max(avb_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
avb_df = avb_df.drop(['symbol'], axis=1)
avb_df['data_date'] = pd.to_datetime(avb_df['data_date'])
avb_df.set_index('data_date', inplace=True)
avb_df.index = pd.DatetimeIndex(avb_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = avb_df.head(round(len(avb_df)*.75))
test = avb_df.tail(round(len(avb_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 7, 28)
...
# RMSE for Model (8, 1, 2): 4.039894250352925
# FIT THE MODEL
...
model = ARIMA(avb_df, order=(8, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
