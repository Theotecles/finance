...
avgo_df = pd.read_sql(query, conn)
print(avgo_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
avgo_dates = []
for date in avgo_df['data_date']:
    avgo_dates.append(datetime.strptime(date, '%Y-%m-%d'))

avgo_df['data_date'] = avgo_dates
symbol = avgo_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(avgo_dates, avgo_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(avgo_df['adjusted_close']), max(avgo_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
avgo_df = avgo_df.drop(['symbol'], axis=1)
avgo_df['data_date'] = pd.to_datetime(avgo_df['data_date'])
avgo_df.set_index('data_date', inplace=True)
avgo_df.index = pd.DatetimeIndex(avgo_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = avgo_df.head(round(len(avgo_df)*.75))
test = avgo_df.tail(round(len(avgo_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2020, 1, 29)
...
# RMSE for Model (8, 2, 1): 90.46262770056626
# FIT THE MODEL
...
model = ARIMA(avgo_df, order=(8, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
