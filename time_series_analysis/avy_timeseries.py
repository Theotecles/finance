...
avy_df = pd.read_sql(query, conn)
print(avy_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
avy_dates = []
for date in avy_df['data_date']:
    avy_dates.append(datetime.strptime(date, '%Y-%m-%d'))

avy_df['data_date'] = avy_dates
symbol = avy_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(avy_dates, avy_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(avy_df['adjusted_close']), max(avy_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
avy_df = avy_df.drop(['symbol'], axis=1)
avy_df['data_date'] = pd.to_datetime(avy_df['data_date'])
avy_df.set_index('data_date', inplace=True)
avy_df.index = pd.DatetimeIndex(avy_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = avy_df.head(round(len(avy_df)*.75))
test = avy_df.tail(round(len(avy_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 1, 1): 12.862301868796322
# FIT THE MODEL
...
model = ARIMA(avy_df, order=(0, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
