...
awk_df = pd.read_sql(query, conn)
print(awk_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
awk_dates = []
for date in awk_df['data_date']:
    awk_dates.append(datetime.strptime(date, '%Y-%m-%d'))

awk_df['data_date'] = awk_dates
symbol = awk_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(awk_dates, awk_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(awk_df['adjusted_close']), max(awk_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
awk_df = awk_df.drop(['symbol'], axis=1)
awk_df['data_date'] = pd.to_datetime(awk_df['data_date'])
awk_df.set_index('data_date', inplace=True)
awk_df.index = pd.DatetimeIndex(awk_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = awk_df.head(round(len(awk_df)*.75))
test = awk_df.tail(round(len(awk_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2018, 5, 16)
...
# RMSE for Model (4, 2, 2): 19.202143994600572
# FIT THE MODEL
...
model = ARIMA(awk_df, order=(4, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(awk_df['adjusted_close']), max(projections))
