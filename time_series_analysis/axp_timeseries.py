...
axp_df = pd.read_sql(query, conn)
print(axp_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
axp_dates = []
for date in axp_df['data_date']:
    axp_dates.append(datetime.strptime(date, '%Y-%m-%d'))

axp_df['data_date'] = axp_dates
symbol = axp_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(axp_dates, axp_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(axp_df['adjusted_close']), max(axp_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
axp_df = axp_df.drop(['symbol'], axis=1)
axp_df['data_date'] = pd.to_datetime(axp_df['data_date'])
axp_df.set_index('data_date', inplace=True)
axp_df.index = pd.DatetimeIndex(axp_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = axp_df.head(round(len(axp_df)*.75))
test = axp_df.tail(round(len(axp_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 2, 1): 30.29020577033975
# FIT THE MODEL
...
model = ARIMA(axp_df, order=(10, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(axp_df['adjusted_close']), max(projections))
