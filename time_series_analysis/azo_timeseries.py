...
azo_df = pd.read_sql(query, conn)
print(azo_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
azo_dates = []
for date in azo_df['data_date']:
    azo_dates.append(datetime.strptime(date, '%Y-%m-%d'))

azo_df['data_date'] = azo_dates
symbol = azo_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(azo_dates, azo_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(azo_df['adjusted_close']), max(azo_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
azo_df = azo_df.drop(['symbol'], axis=1)
azo_df['data_date'] = pd.to_datetime(azo_df['data_date'])
azo_df.set_index('data_date', inplace=True)
azo_df.index = pd.DatetimeIndex(azo_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = azo_df.head(round(len(azo_df)*.75))
test = azo_df.tail(round(len(azo_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2020, 10, 27)
...
# RMSE for Model (2, 2, 1): 142.3427184690566
# FIT THE MODEL
...
model = ARIMA(azo_df, order=(2, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
