...
bac_df = pd.read_sql(query, conn)
print(bac_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
bac_dates = []
for date in bac_df['data_date']:
    bac_dates.append(datetime.strptime(date, '%Y-%m-%d'))

bac_df['data_date'] = bac_dates
symbol = bac_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(bac_dates, bac_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(bac_df['adjusted_close']), max(bac_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
bac_df = bac_df.drop(['symbol'], axis=1)
bac_df['data_date'] = pd.to_datetime(bac_df['data_date'])
bac_df.set_index('data_date', inplace=True)
bac_df.index = pd.DatetimeIndex(bac_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = bac_df.head(round(len(bac_df)*.75))
test = bac_df.tail(round(len(bac_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 1, 1): 6.69050599100478
# FIT THE MODEL
...
model = ARIMA(bac_df, order=(10, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
