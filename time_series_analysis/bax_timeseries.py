...
bax_df = pd.read_sql(query, conn)
print(bax_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
bax_dates = []
for date in bax_df['data_date']:
    bax_dates.append(datetime.strptime(date, '%Y-%m-%d'))

bax_df['data_date'] = bax_dates
symbol = bax_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(bax_dates, bax_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(bax_df['adjusted_close']), max(bax_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
bax_df = bax_df.drop(['symbol'], axis=1)
bax_df['data_date'] = pd.to_datetime(bax_df['data_date'])
bax_df.set_index('data_date', inplace=True)
bax_df.index = pd.DatetimeIndex(bax_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = bax_df.head(round(len(bax_df)*.75))
test = bax_df.tail(round(len(bax_df)*.25))
print(test.head())
print(test.tail())
...
# MSE for Model (6, 0, 2): 3.41743128117095
# FIT THE MODEL
...
model = ARIMA(bax_df, order=(6, 0, 2))
model_fit = model.fit()
print(model_fit.summary())
