...
bby_df = pd.read_sql(query, conn)
print(bby_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
bby_dates = []
for date in bby_df['data_date']:
    bby_dates.append(datetime.strptime(date, '%Y-%m-%d'))

bby_df['data_date'] = bby_dates
symbol = bby_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(bby_dates, bby_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(bby_df['adjusted_close']), max(bby_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
bby_df = bby_df.drop(['symbol'], axis=1)
bby_df['data_date'] = pd.to_datetime(bby_df['data_date'])
bby_df.set_index('data_date', inplace=True)
bby_df.index = pd.DatetimeIndex(bby_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = bby_df.head(round(len(bby_df)*.75))
test = bby_df.tail(round(len(bby_df)*.25))
print(test.head())
print(test.tail())
...
model = ARIMA(bby_df, order=(8, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
