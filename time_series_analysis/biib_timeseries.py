...
biib_df = pd.read_sql(query, conn)
print(biib_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
biib_dates = []
for date in biib_df['data_date']:
    biib_dates.append(datetime.strptime(date, '%Y-%m-%d'))

biib_df['data_date'] = biib_dates
symbol = biib_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(biib_dates, biib_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(biib_df['adjusted_close']), max(biib_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
biib_df = biib_df.drop(['symbol'], axis=1)
biib_df['data_date'] = pd.to_datetime(biib_df['data_date'])
biib_df.set_index('data_date', inplace=True)
biib_df.index = pd.DatetimeIndex(biib_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = biib_df.head(round(len(biib_df)*.75))
test = biib_df.tail(round(len(biib_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2019, 10, 28)
...
# RMSE for Model (4, 2, 2): 32.06591066238749
# FIT THE MODEL
...
model = ARIMA(biib_df, order=(4, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
