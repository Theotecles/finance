...
bio_df = pd.read_sql(query, conn)
print(bio_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
bio_dates = []
for date in bio_df['data_date']:
    bio_dates.append(datetime.strptime(date, '%Y-%m-%d'))

bio_df['data_date'] = bio_dates
symbol = bio_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(bio_dates, bio_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(bio_df['adjusted_close']), max(bio_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
bio_df = bio_df.drop(['symbol'], axis=1)
bio_df['data_date'] = pd.to_datetime(bio_df['data_date'])
bio_df.set_index('data_date', inplace=True)
bio_df.index = pd.DatetimeIndex(bio_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = bio_df.head(round(len(bio_df)*.75))
test = bio_df.tail(round(len(bio_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 1, 26)
...
# RMSE for Model (8, 1, 0): 102.3911370579667
# FIT THE MODEL
...
model = ARIMA(bio_df, order=(8, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
