...
bk_df = pd.read_sql(query, conn)
print(bk_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
bk_dates = []
for date in bk_df['data_date']:
    bk_dates.append(datetime.strptime(date, '%Y-%m-%d'))

bk_df['data_date'] = bk_dates
symbol = bk_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(bk_dates, bk_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(bk_df['adjusted_close']), max(bk_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
bk_df = bk_df.drop(['symbol'], axis=1)
bk_df['data_date'] = pd.to_datetime(bk_df['data_date'])
bk_df.set_index('data_date', inplace=True)
bk_df.index = pd.DatetimeIndex(bk_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = bk_df.head(round(len(bk_df)*.75))
test = bk_df.tail(round(len(bk_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 0, 2): 6.566756834643707
# FIT THE MODEL
...
model = ARIMA(bk_df, order=(10, 0, 2))
model_fit = model.fit()
print(model_fit.summary())
