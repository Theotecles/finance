...
query = '''
    ...
    WHERE symbol = 'BLK'
    AND data_date >= '2013-01-01'
    ORDER BY data_date ASC
    '''

# PULL IN THE DATA USING PANDAS
blk_df = pd.read_sql(query, conn)
print(blk_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
blk_dates = []
for date in blk_df['data_date']:
    blk_dates.append(datetime.strptime(date, '%Y-%m-%d'))

blk_df['data_date'] = blk_dates
symbol = blk_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(blk_dates, blk_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(blk_df['adjusted_close']), max(blk_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
blk_df = blk_df.drop(['symbol'], axis=1)
blk_df['data_date'] = pd.to_datetime(blk_df['data_date'])
blk_df.set_index('data_date', inplace=True)
blk_df.index = pd.DatetimeIndex(blk_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = blk_df.head(round(len(blk_df)*.75))
test = blk_df.tail(round(len(blk_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2019, 7, 30)
...
# RMSE for Model (0, 2, 1): 134.04396559060444
# FIT THE MODEL
...
model = ARIMA(blk_df, order=(0, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
