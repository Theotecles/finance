...
bll_df = pd.read_sql(query, conn)
print(bll_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
bll_dates = []
for date in bll_df['data_date']:
    bll_dates.append(datetime.strptime(date, '%Y-%m-%d'))

bll_df['data_date'] = bll_dates
symbol = bll_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(bll_dates, bll_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(bll_df['adjusted_close']), max(bll_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
bll_df = bll_df.drop(['symbol'], axis=1)
bll_df['data_date'] = pd.to_datetime(bll_df['data_date'])
bll_df.set_index('data_date', inplace=True)
bll_df.index = pd.DatetimeIndex(bll_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = bll_df.head(round(len(bll_df)*.75))
test = bll_df.tail(round(len(bll_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 1, 2): 7.477904983057839
# FIT THE MODEL
...
model = ARIMA(bll_df, order=(10, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
