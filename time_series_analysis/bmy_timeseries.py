...
bmy_df = pd.read_sql(query, conn)
print(bmy_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
bmy_dates = []
for date in bmy_df['data_date']:
    bmy_dates.append(datetime.strptime(date, '%Y-%m-%d'))

bmy_df['data_date'] = bmy_dates
symbol = bmy_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(bmy_dates, bmy_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(bmy_df['adjusted_close']), max(bmy_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
bmy_df = bmy_df.drop(['symbol'], axis=1)
bmy_df['data_date'] = pd.to_datetime(bmy_df['data_date'])
bmy_df.set_index('data_date', inplace=True)
bmy_df.index = pd.DatetimeIndex(bmy_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = bmy_df.head(round(len(bmy_df)*.75))
test = bmy_df.tail(round(len(bmy_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2020, 4, 30)
...
# RMSE for Model (4, 2, 2): 3.3538051544799075
# FIT THE MODEL
...
model = ARIMA(bmy_df, order=(4, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
