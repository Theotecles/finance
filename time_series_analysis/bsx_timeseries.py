...
bsx_df = pd.read_sql(query, conn)
print(bsx_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
bsx_dates = []
for date in bsx_df['data_date']:
    bsx_dates.append(datetime.strptime(date, '%Y-%m-%d'))

bsx_df['data_date'] = bsx_dates
symbol = bsx_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(bsx_dates, bsx_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(bsx_df['adjusted_close']), max(bsx_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
bsx_df = bsx_df.drop(['symbol'], axis=1)
bsx_df['data_date'] = pd.to_datetime(bsx_df['data_date'])
bsx_df.set_index('data_date', inplace=True)
bsx_df.index = pd.DatetimeIndex(bsx_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = bsx_df.head(round(len(bsx_df)*.75))
test = bsx_df.tail(round(len(bsx_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 1, 1): 5.733850476713542
# FIT THE MODEL
...
model = ARIMA(bsx_df, order=(0, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
