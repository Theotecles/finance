...
bwa_df = pd.read_sql(query, conn)
print(bwa_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
bwa_dates = []
for date in bwa_df['data_date']:
    bwa_dates.append(datetime.strptime(date, '%Y-%m-%d'))

bwa_df['data_date'] = bwa_dates
symbol = bwa_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(bwa_dates, bwa_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(bwa_df['adjusted_close']), max(bwa_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
bwa_df = bwa_df.drop(['symbol'], axis=1)
bwa_df['data_date'] = pd.to_datetime(bwa_df['data_date'])
bwa_df.set_index('data_date', inplace=True)
bwa_df.index = pd.DatetimeIndex(bwa_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = bwa_df.head(round(len(bwa_df)*.75))
test = bwa_df.tail(round(len(bwa_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 2, 1): 4.006732271471436
# FIT THE MODEL
...
model = ARIMA(bwa_df, order=(0, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
