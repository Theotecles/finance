...
bxp_df = pd.read_sql(query, conn)
print(bxp_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
bxp_dates = []
for date in bxp_df['data_date']:
    bxp_dates.append(datetime.strptime(date, '%Y-%m-%d'))

bxp_df['data_date'] = bxp_dates
symbol = bxp_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(bxp_dates, bxp_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(bxp_df['adjusted_close']), max(bxp_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
bxp_df = bxp_df.drop(['symbol'], axis=1)
bxp_df['data_date'] = pd.to_datetime(bxp_df['data_date'])
bxp_df.set_index('data_date', inplace=True)
bxp_df.index = pd.DatetimeIndex(bxp_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = bxp_df.head(round(len(bxp_df)*.75))
test = bxp_df.tail(round(len(bxp_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 5, 20)
...
# RMSE for Model (0, 1, 0): 7.554550179928158
# FIT THE MODEL
...
model = ARIMA(bxp_df, order=(0, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
