...
cag_df = pd.read_sql(query, conn)
print(cag_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cag_dates = []
for date in cag_df['data_date']:
    cag_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cag_df['data_date'] = cag_dates
symbol = cag_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cag_dates, cag_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cag_df['adjusted_close']), max(cag_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cag_df = cag_df.drop(['symbol'], axis=1)
cag_df['data_date'] = pd.to_datetime(cag_df['data_date'])
cag_df.set_index('data_date', inplace=True)
cag_df.index = pd.DatetimeIndex(cag_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cag_df.head(round(len(cag_df)*.75))
test = cag_df.tail(round(len(cag_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 1, 2): 2.043509425923494
# FIT THE MODEL
...
model = ARIMA(cag_df, order=(10, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
