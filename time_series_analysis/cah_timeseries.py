...
cah_df = pd.read_sql(query, conn)
print(cah_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cah_dates = []
for date in cah_df['data_date']:
    cah_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cah_df['data_date'] = cah_dates
symbol = cah_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cah_dates, cah_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cah_df['adjusted_close']), max(cah_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cah_df = cah_df.drop(['symbol'], axis=1)
cah_df['data_date'] = pd.to_datetime(cah_df['data_date'])
cah_df.set_index('data_date', inplace=True)
cah_df.index = pd.DatetimeIndex(cah_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cah_df.head(round(len(cah_df)*.75))
test = cah_df.tail(round(len(cah_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 1, 0): 3.5417357261691613
# FIT THE MODEL
...
model = ARIMA(cah_df, order=(0, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
