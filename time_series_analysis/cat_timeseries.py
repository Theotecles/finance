...
cat_df = pd.read_sql(query, conn)
print(cat_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cat_dates = []
for date in cat_df['data_date']:
    cat_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cat_df['data_date'] = cat_dates
symbol = cat_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cat_dates, cat_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cat_df['adjusted_close']), max(cat_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cat_df = cat_df.drop(['symbol'], axis=1)
cat_df['data_date'] = pd.to_datetime(cat_df['data_date'])
cat_df.set_index('data_date', inplace=True)
cat_df.index = pd.DatetimeIndex(cat_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cat_df.head(round(len(cat_df)*.75))
test = cat_df.tail(round(len(cat_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (1, 0, 0): 10.051296002336102
# FIT THE MODEL
...
model = ARIMA(cat_df, order=(1, 0, 0))
model_fit = model.fit()
print(model_fit.summary())
