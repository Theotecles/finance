...
cb_df = pd.read_sql(query, conn)
print(cb_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cb_dates = []
for date in cb_df['data_date']:
    cb_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cb_df['data_date'] = cb_dates
symbol = cb_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cb_dates, cb_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cb_df['adjusted_close']), max(cb_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cb_df = cb_df.drop(['symbol'], axis=1)
cb_df['data_date'] = pd.to_datetime(cb_df['data_date'])
cb_df.set_index('data_date', inplace=True)
cb_df.index = pd.DatetimeIndex(cb_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cb_df.head(round(len(cb_df)*.75))
test = cb_df.tail(round(len(cb_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 1, 1): 10.697698403438753
# FIT THE MODEL
...
model = ARIMA(cb_df, order=(10, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
