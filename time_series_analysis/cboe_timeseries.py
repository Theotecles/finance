...
cboe_df = pd.read_sql(query, conn)
print(cboe_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cboe_dates = []
for date in cboe_df['data_date']:
    cboe_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cboe_df['data_date'] = cboe_dates
symbol = cboe_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cboe_dates, cboe_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cboe_df['adjusted_close']), max(cboe_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cboe_df = cboe_df.drop(['symbol'], axis=1)
cboe_df['data_date'] = pd.to_datetime(cboe_df['data_date'])
cboe_df.set_index('data_date', inplace=True)
cboe_df.index = pd.DatetimeIndex(cboe_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cboe_df.head(round(len(cboe_df)*.75))
test = cboe_df.tail(round(len(cboe_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (4, 1, 0): 6.353591812442619
# FIT THE MODEL
...
model = ARIMA(cboe_df, order=(4, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
