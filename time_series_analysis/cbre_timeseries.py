...
cbre_df = pd.read_sql(query, conn)
print(cbre_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cbre_dates = []
for date in cbre_df['data_date']:
    cbre_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cbre_df['data_date'] = cbre_dates
symbol = cbre_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cbre_dates, cbre_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cbre_df['adjusted_close']), max(cbre_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cbre_df = cbre_df.drop(['symbol'], axis=1)
cbre_df['data_date'] = pd.to_datetime(cbre_df['data_date'])
cbre_df.set_index('data_date', inplace=True)
cbre_df.index = pd.DatetimeIndex(cbre_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cbre_df.head(round(len(cbre_df)*.75))
test = cbre_df.tail(round(len(cbre_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 1): 17.910797570213912
# FIT THE MODEL
...
model = ARIMA(cbre_df, order=(6, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
