...
cci_dates = []
for date in cci_df['data_date']:
    cci_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cci_df['data_date'] = cci_dates
symbol = cci_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cci_dates, cci_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cci_df['adjusted_close']), max(cci_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cci_df = cci_df.drop(['symbol'], axis=1)
cci_df['data_date'] = pd.to_datetime(cci_df['data_date'])
cci_df.set_index('data_date', inplace=True)
cci_df.index = pd.DatetimeIndex(cci_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cci_df.head(round(len(cci_df)*.75))
test = cci_df.tail(round(len(cci_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (8, 2, 1): 24.398120638136394
# FIT THE MODEL
...
model = ARIMA(cci_df, order=(8, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
