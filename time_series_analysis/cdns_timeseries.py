...
cdns_df = pd.read_sql(query, conn)
print(cdns_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cdns_dates = []
for date in cdns_df['data_date']:
    cdns_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cdns_df['data_date'] = cdns_dates
symbol = cdns_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cdns_dates, cdns_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cdns_df['adjusted_close']), max(cdns_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cdns_df = cdns_df.drop(['symbol'], axis=1)
cdns_df['data_date'] = pd.to_datetime(cdns_df['data_date'])
cdns_df.set_index('data_date', inplace=True)
cdns_df.index = pd.DatetimeIndex(cdns_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cdns_df.head(round(len(cdns_df)*.75))
test = cdns_df.tail(round(len(cdns_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 0): 16.466691194391412
# FIT THE MODEL
...
model = ARIMA(cdns_df, order=(6, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
