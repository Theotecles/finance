...
cdw_df = pd.read_sql(query, conn)
print(cdw_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cdw_dates = []
for date in cdw_df['data_date']:
    cdw_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cdw_df['data_date'] = cdw_dates
symbol = cdw_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cdw_dates, cdw_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cdw_df['adjusted_close']), max(cdw_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cdw_df = cdw_df.drop(['symbol'], axis=1)
cdw_df['data_date'] = pd.to_datetime(cdw_df['data_date'])
cdw_df.set_index('data_date', inplace=True)
cdw_df.index = pd.DatetimeIndex(cdw_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cdw_df.head(round(len(cdw_df)*.75))
test = cdw_df.tail(round(len(cdw_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 9, 13)
...
# RMSE for Model (1, 2, 2): 13.84817780292348
# FIT THE MODEL
...
model = ARIMA(cdw_df, order=(1, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
