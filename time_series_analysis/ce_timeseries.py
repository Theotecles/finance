...
ce_df = pd.read_sql(query, conn)
print(ce_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
ce_dates = []
for date in ce_df['data_date']:
    ce_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ce_df['data_date'] = ce_dates
symbol = ce_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ce_dates, ce_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ce_df['adjusted_close']), max(ce_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ce_df = ce_df.drop(['symbol'], axis=1)
ce_df['data_date'] = pd.to_datetime(ce_df['data_date'])
ce_df.set_index('data_date', inplace=True)
ce_df.index = pd.DatetimeIndex(ce_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ce_df.head(round(len(ce_df)*.75))
test = ce_df.tail(round(len(ce_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 2): 30.5630049485801
# FIT THE MODEL
