...
cern_df = pd.read_sql(query, conn)
print(cern_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cern_dates = []
for date in cern_df['data_date']:
    cern_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cern_df['data_date'] = cern_dates
symbol = cern_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cern_dates, cern_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cern_df['adjusted_close']), max(cern_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cern_df = cern_df.drop(['symbol'], axis=1)
cern_df['data_date'] = pd.to_datetime(cern_df['data_date'])
cern_df.set_index('data_date', inplace=True)
cern_df.index = pd.DatetimeIndex(cern_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cern_df.head(round(len(cern_df)*.75))
test = cern_df.tail(round(len(cern_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (4, 2, 1): 4.220577562816081
# FIT THE MODEL
...
model = ARIMA(cern_df, order=(4, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
