...
cf_df = pd.read_sql(query, conn)
print(cf_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cf_dates = []
for date in cf_df['data_date']:
    cf_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cf_df['data_date'] = cf_dates
symbol = cf_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cf_dates, cf_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cf_df['adjusted_close']), max(cf_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cf_df = cf_df.drop(['symbol'], axis=1)
cf_df['data_date'] = pd.to_datetime(cf_df['data_date'])
cf_df.set_index('data_date', inplace=True)
cf_df.index = pd.DatetimeIndex(cf_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cf_df.head(round(len(cf_df)*.75))
test = cf_df.tail(round(len(cf_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (8, 0, 2): 4.484585812945723
# FIT THE MODEL
