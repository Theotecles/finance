...
query = '''
    ...
    WHERE symbol = 'CHD'
    AND data_date >= '2020-08-01'
    ORDER BY data_date ASC
    '''

# PULL IN THE DATA USING PANDAS
chd_df = pd.read_sql(query, conn)
print(chd_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
chd_dates = []
for date in chd_df['data_date']:
    chd_dates.append(datetime.strptime(date, '%Y-%m-%d'))

chd_df['data_date'] = chd_dates
symbol = chd_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(chd_dates, chd_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(chd_df['adjusted_close']), max(chd_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
chd_df = chd_df.drop(['symbol'], axis=1)
chd_df['data_date'] = pd.to_datetime(chd_df['data_date'])
chd_df.set_index('data_date', inplace=True)
chd_df.index = pd.DatetimeIndex(chd_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = chd_df.head(round(len(chd_df)*.75))
test = chd_df.tail(round(len(chd_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 6, 23)
...
# RMSE for Model (6, 1, 2): 1.5001366693669937
# FIT THE MODEL
...
model = ARIMA(chd_df, order=(6, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
