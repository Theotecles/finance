...
chrw_df = pd.read_sql(query, conn)
print(chrw_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
chrw_dates = []
for date in chrw_df['data_date']:
    chrw_dates.append(datetime.strptime(date, '%Y-%m-%d'))

chrw_df['data_date'] = chrw_dates
symbol = chrw_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(chrw_dates, chrw_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(chrw_df['adjusted_close']), max(chrw_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
chrw_df = chrw_df.drop(['symbol'], axis=1)
chrw_df['data_date'] = pd.to_datetime(chrw_df['data_date'])
chrw_df.set_index('data_date', inplace=True)
chrw_df.index = pd.DatetimeIndex(chrw_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = chrw_df.head(round(len(chrw_df)*.75))
test = chrw_df.tail(round(len(chrw_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2018, 4, 30)
...
# RMSE for Model (10, 1, 1): 8.983769327231514
# FIT THE MODEL
...
model = ARIMA(chrw_df, order=(10, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
