...
chtr_df = pd.read_sql(query, conn)
print(chtr_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
chtr_dates = []
for date in chtr_df['data_date']:
    chtr_dates.append(datetime.strptime(date, '%Y-%m-%d'))

chtr_df['data_date'] = chtr_dates
symbol = chtr_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(chtr_dates, chtr_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(chtr_df['adjusted_close']), max(chtr_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
chtr_df = chtr_df.drop(['symbol'], axis=1)
chtr_df['data_date'] = pd.to_datetime(chtr_df['data_date'])
chtr_df.set_index('data_date', inplace=True)
chtr_df.index = pd.DatetimeIndex(chtr_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = chtr_df.head(round(len(chtr_df)*.75))
test = chtr_df.tail(round(len(chtr_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 0): 99.17037114951907
# FIT THE MODEL
...
model = ARIMA(chtr_df, order=(6, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
