...
ci_df = pd.read_sql(query, conn)
print(ci_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
ci_dates = []
for date in ci_df['data_date']:
    ci_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ci_df['data_date'] = ci_dates
symbol = ci_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ci_dates, ci_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ci_df['adjusted_close']), max(ci_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ci_df = ci_df.drop(['symbol'], axis=1)
ci_df['data_date'] = pd.to_datetime(ci_df['data_date'])
ci_df.set_index('data_date', inplace=True)
ci_df.index = pd.DatetimeIndex(ci_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ci_df.head(round(len(ci_df)*.75))
test = ci_df.tail(round(len(ci_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 2, 0): 20.05891742887697
# FIT THE MODEL
...
model = ARIMA(ci_df, order=(10, 2, 0))
model_fit = model.fit()
print(model_fit.summary())
