...
query = '''
    ...
    WHERE symbol = 'CINF'
    AND data_date >= '2020-06-01'
    ORDER BY data_date ASC
    '''

# PULL IN THE DATA USING PANDAS
cinf_df = pd.read_sql(query, conn)
print(cinf_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cinf_dates = []
for date in cinf_df['data_date']:
    cinf_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cinf_df['data_date'] = cinf_dates
symbol = cinf_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cinf_dates, cinf_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cinf_df['adjusted_close']), max(cinf_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cinf_df = cinf_df.drop(['symbol'], axis=1)
cinf_df['data_date'] = pd.to_datetime(cinf_df['data_date'])
cinf_df.set_index('data_date', inplace=True)
cinf_df.index = pd.DatetimeIndex(cinf_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cinf_df.head(round(len(cinf_df)*.75))
test = cinf_df.tail(round(len(cinf_df)*.25))
print(test.head())
print(test.tail())
...
# RRMSE for Model (4, 0, 2): 3.766121983838632
# FIT THE MODEL
...
model = ARIMA(cinf_df, order=(4, 0, 2))
model_fit = model.fit()
print(model_fit.summary())
