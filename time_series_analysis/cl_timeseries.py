...
cl_df = pd.read_sql(query, conn)
print(cl_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cl_dates = []
for date in cl_df['data_date']:
    cl_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cl_df['data_date'] = cl_dates
symbol = cl_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cl_dates, cl_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cl_df['adjusted_close']), max(cl_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cl_df = cl_df.drop(['symbol'], axis=1)
cl_df['data_date'] = pd.to_datetime(cl_df['data_date'])
cl_df.set_index('data_date', inplace=True)
cl_df.index = pd.DatetimeIndex(cl_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cl_df.head(round(len(cl_df)*.75))
test = cl_df.tail(round(len(cl_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 1, 1): 12.06011639719772
# FIT THE MODEL
...
model = ARIMA(cl_df, order=(10, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
