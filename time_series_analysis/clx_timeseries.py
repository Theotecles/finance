...
clx_df = pd.read_sql(query, conn)
print(clx_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
clx_dates = []
for date in clx_df['data_date']:
    clx_dates.append(datetime.strptime(date, '%Y-%m-%d'))

clx_df['data_date'] = clx_dates
symbol = clx_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(clx_dates, clx_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(clx_df['adjusted_close']), max(clx_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
clx_df = clx_df.drop(['symbol'], axis=1)
clx_df['data_date'] = pd.to_datetime(clx_df['data_date'])
clx_df.set_index('data_date', inplace=True)
clx_df.index = pd.DatetimeIndex(clx_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = clx_df.head(round(len(clx_df)*.75))
test = clx_df.tail(round(len(clx_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 6, 14)
...
# RMSE for Model (0, 1, 1): 7.550327171765969
# FIT THE MODEL
...
model = ARIMA(clx_df, order=(0, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
