...
cma_df = pd.read_sql(query, conn)
print(cma_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cma_dates = []
for date in cma_df['data_date']:
    cma_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cma_df['data_date'] = cma_dates
symbol = cma_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cma_dates, cma_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cma_df['adjusted_close']), max(cma_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cma_df = cma_df.drop(['symbol'], axis=1)
cma_df['data_date'] = pd.to_datetime(cma_df['data_date'])
cma_df.set_index('data_date', inplace=True)
cma_df.index = pd.DatetimeIndex(cma_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cma_df.head(round(len(cma_df)*.75))
test = cma_df.tail(round(len(cma_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 0, 2): 15.201272886537202
# FIT THE MODEL
...
model = ARIMA(cma_df, order=(10, 0, 2))
model_fit = model.fit()
print(model_fit.summary())
