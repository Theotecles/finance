...
cme_df = pd.read_sql(query, conn)
print(cme_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cme_dates = []
for date in cme_df['data_date']:
    cme_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cme_df['data_date'] = cme_dates
symbol = cme_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cme_dates, cme_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cme_df['adjusted_close']), max(cme_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cme_df = cme_df.drop(['symbol'], axis=1)
cme_df['data_date'] = pd.to_datetime(cme_df['data_date'])
cme_df.set_index('data_date', inplace=True)
cme_df.index = pd.DatetimeIndex(cme_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cme_df.head(round(len(cme_df)*.75))
test = cme_df.tail(round(len(cme_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (8, 1, 2): 9.407278803051643
# FIT THE MODEL
...
model = ARIMA(cme_df, order=(8, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
