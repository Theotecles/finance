...
cmg_df = pd.read_sql(query, conn)
print(cmg_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cmg_dates = []
for date in cmg_df['data_date']:
    cmg_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cmg_df['data_date'] = cmg_dates
symbol = cmg_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cmg_dates, cmg_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cmg_df['adjusted_close']), max(cmg_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cmg_df = cmg_df.drop(['symbol'], axis=1)
cmg_df['data_date'] = pd.to_datetime(cmg_df['data_date'])
cmg_df.set_index('data_date', inplace=True)
cmg_df.index = pd.DatetimeIndex(cmg_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cmg_df.head(round(len(cmg_df)*.75))
test = cmg_df.tail(round(len(cmg_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (1, 1, 0): 340.6964488381778
# FIT THE MODEL
...
model = ARIMA(cmg_df, order=(1, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
