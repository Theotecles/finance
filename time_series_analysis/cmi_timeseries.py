...
cmi_df = pd.read_sql(query, conn)
print(cmi_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cmi_dates = []
for date in cmi_df['data_date']:
    cmi_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cmi_df['data_date'] = cmi_dates
symbol = cmi_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cmi_dates, cmi_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cmi_df['adjusted_close']), max(cmi_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cmi_df = cmi_df.drop(['symbol'], axis=1)
cmi_df['data_date'] = pd.to_datetime(cmi_df['data_date'])
cmi_df.set_index('data_date', inplace=True)
cmi_df.index = pd.DatetimeIndex(cmi_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cmi_df.head(round(len(cmi_df)*.75))
test = cmi_df.tail(round(len(cmi_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2020, 5, 26)
...
# RMSE for Model (8, 0, 2): 7.190690713100711
# FIT THE MODEL
...
model = ARIMA(cmi_df, order=(8, 0, 2))
model_fit = model.fit()
print(model_fit.summary())
