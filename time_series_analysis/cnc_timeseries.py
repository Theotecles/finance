...
cnc_df = pd.read_sql(query, conn)
print(cnc_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cnc_dates = []
for date in cnc_df['data_date']:
    cnc_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cnc_df['data_date'] = cnc_dates
symbol = cnc_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cnc_dates, cnc_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cnc_df['adjusted_close']), max(cnc_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cnc_df = cnc_df.drop(['symbol'], axis=1)
cnc_df['data_date'] = pd.to_datetime(cnc_df['data_date'])
cnc_df.set_index('data_date', inplace=True)
cnc_df.index = pd.DatetimeIndex(cnc_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cnc_df.head(round(len(cnc_df)*.75))
test = cnc_df.tail(round(len(cnc_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 0, 2): 7.1001925169524265
# FIT THE MODEL
...
model = ARIMA(cnc_df, order=(0, 0, 2))
model_fit = model.fit()
print(model_fit.summary())
