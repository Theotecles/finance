...
query = '''
    ...
    WHERE symbol = 'CNP'
    AND data_date >= '2004-01-01'
    ORDER BY data_date ASC
    '''

# PULL IN THE DATA USING PANDAS
cnp_df = pd.read_sql(query, conn)
print(cnp_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cnp_dates = []
for date in cnp_df['data_date']:
    cnp_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cnp_df['data_date'] = cnp_dates
symbol = cnp_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cnp_dates, cnp_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cnp_df['adjusted_close']), max(cnp_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cnp_df = cnp_df.drop(['symbol'], axis=1)
cnp_df['data_date'] = pd.to_datetime(cnp_df['data_date'])
cnp_df.set_index('data_date', inplace=True)
cnp_df.index = pd.DatetimeIndex(cnp_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cnp_df.head(round(len(cnp_df)*.75))
test = cnp_df.tail(round(len(cnp_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 1, 2): 3.1704807067109724
# FIT THE MODEL
...
model = ARIMA(cnp_df, order=(10, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
