...
cof_df = pd.read_sql(query, conn)
print(cof_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cof_dates = []
for date in cof_df['data_date']:
    cof_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cof_df['data_date'] = cof_dates
symbol = cof_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cof_dates, cof_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cof_df['adjusted_close']), max(cof_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cof_df = cof_df.drop(['symbol'], axis=1)
cof_df['data_date'] = pd.to_datetime(cof_df['data_date'])
cof_df.set_index('data_date', inplace=True)
cof_df.index = pd.DatetimeIndex(cof_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cof_df.head(round(len(cof_df)*.75))
test = cof_df.tail(round(len(cof_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 1): 7.66661760382895
# FIT THE MODEL
...
model = ARIMA(cof_df, order=(6, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
