...
cog_df = pd.read_sql(query, conn)
print(cog_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cog_dates = []
for date in cog_df['data_date']:
    cog_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cog_df['data_date'] = cog_dates
symbol = cog_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cog_dates, cog_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cog_df['adjusted_close']), max(cog_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cog_df = cog_df.drop(['symbol'], axis=1)
cog_df['data_date'] = pd.to_datetime(cog_df['data_date'])
cog_df.set_index('data_date', inplace=True)
cog_df.index = pd.DatetimeIndex(cog_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cog_df.head(round(len(cog_df)*.75))
test = cog_df.tail(round(len(cog_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2019, 10, 24)
...
# RMSE for Model (8, 1, 0): 1.6086773443223097
# FIT THE MODEL
...
model = ARIMA(cog_df, order=(8, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
