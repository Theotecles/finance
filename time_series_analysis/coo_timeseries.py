...
coo_df = pd.read_sql(query, conn)
print(coo_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
coo_dates = []
for date in coo_df['data_date']:
    coo_dates.append(datetime.strptime(date, '%Y-%m-%d'))

coo_df['data_date'] = coo_dates
symbol = coo_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(coo_dates, coo_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(coo_df['adjusted_close']), max(coo_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
coo_df = coo_df.drop(['symbol'], axis=1)
coo_df['data_date'] = pd.to_datetime(coo_df['data_date'])
coo_df.set_index('data_date', inplace=True)
coo_df.index = pd.DatetimeIndex(coo_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = coo_df.head(round(len(coo_df)*.75))
test = coo_df.tail(round(len(coo_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 1, 0): 82.40213228490815
# FIT THE MODEL
...
model = ARIMA(coo_df, order=(0, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
