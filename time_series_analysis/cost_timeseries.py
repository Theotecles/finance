...
cost_dates = []
for date in cost_df['data_date']:
    cost_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cost_df['data_date'] = cost_dates
symbol = cost_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cost_dates, cost_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cost_df['adjusted_close']), max(cost_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cost_df = cost_df.drop(['symbol'], axis=1)
cost_df['data_date'] = pd.to_datetime(cost_df['data_date'])
cost_df.set_index('data_date', inplace=True)
cost_df.index = pd.DatetimeIndex(cost_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cost_df.head(round(len(cost_df)*.75))
test = cost_df.tail(round(len(cost_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2018, 7, 31)
...
# RMSE for Model (4, 2, 2): 52.822418786987136
# FIT THE MODEL
...
model = ARIMA(cost_df, order=(4, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
