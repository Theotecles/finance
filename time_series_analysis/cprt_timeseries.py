...
cprt_df = pd.read_sql(query, conn)
print(cprt_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cprt_dates = []
for date in cprt_df['data_date']:
    cprt_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cprt_df['data_date'] = cprt_dates
symbol = cprt_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cprt_dates, cprt_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cprt_df['adjusted_close']), max(cprt_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cprt_df = cprt_df.drop(['symbol'], axis=1)
cprt_df['data_date'] = pd.to_datetime(cprt_df['data_date'])
cprt_df.set_index('data_date', inplace=True)
cprt_df.index = pd.DatetimeIndex(cprt_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cprt_df.head(round(len(cprt_df)*.75))
test = cprt_df.tail(round(len(cprt_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 0): 20.98805563617027
# FIT THE MODEL
...
model = ARIMA(cprt_df, order=(6, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
