...
csco_df = pd.read_sql(query, conn)
print(csco_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
csco_dates = []
for date in csco_df['data_date']:
    csco_dates.append(datetime.strptime(date, '%Y-%m-%d'))

csco_df['data_date'] = csco_dates
symbol = csco_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(csco_dates, csco_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(csco_df['adjusted_close']), max(csco_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
csco_df = csco_df.drop(['symbol'], axis=1)
csco_df['data_date'] = pd.to_datetime(csco_df['data_date'])
csco_df.set_index('data_date', inplace=True)
csco_df.index = pd.DatetimeIndex(csco_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = csco_df.head(round(len(csco_df)*.75))
test = csco_df.tail(round(len(csco_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (2, 0, 0): 6.092194433726561
# FIT THE MODEL
...
model = ARIMA(csco_df, order=(2, 0, 0))
model_fit = model.fit()
print(model_fit.summary())
