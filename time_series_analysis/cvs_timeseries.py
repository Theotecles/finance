...
cvs_df = pd.read_sql(query, conn)
print(cvs_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
cvs_dates = []
for date in cvs_df['data_date']:
    cvs_dates.append(datetime.strptime(date, '%Y-%m-%d'))

cvs_df['data_date'] = cvs_dates
symbol = cvs_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(cvs_dates, cvs_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(cvs_df['adjusted_close']), max(cvs_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
cvs_df = cvs_df.drop(['symbol'], axis=1)
cvs_df['data_date'] = pd.to_datetime(cvs_df['data_date'])
cvs_df.set_index('data_date', inplace=True)
cvs_df.index = pd.DatetimeIndex(cvs_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = cvs_df.head(round(len(cvs_df)*.75))
test = cvs_df.tail(round(len(cvs_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (1, 0, 1): 10.299027894447537
# FIT THE MODEL
...
model = ARIMA(cvs_df, order=(1, 0, 1))
model_fit = model.fit()
print(model_fit.summary())
