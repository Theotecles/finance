...
czr_df = pd.read_sql(query, conn)
print(czr_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
czr_dates = []
for date in czr_df['data_date']:
    czr_dates.append(datetime.strptime(date, '%Y-%m-%d'))

czr_df['data_date'] = czr_dates
symbol = czr_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(czr_dates, czr_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(czr_df['adjusted_close']), max(czr_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
czr_df = czr_df.drop(['symbol'], axis=1)
czr_df['data_date'] = pd.to_datetime(czr_df['data_date'])
czr_df.set_index('data_date', inplace=True)
czr_df.index = pd.DatetimeIndex(czr_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = czr_df.head(round(len(czr_df)*.75))
test = czr_df.tail(round(len(czr_df)*.25))
print(test.head())
print(test.tail())
...
# MSE for Model (10, 2, 0): 9.044513400487117
# FIT THE MODEL
...
model = ARIMA(czr_df, order=(10, 2, 0))
model_fit = model.fit()
print(model_fit.summary())
