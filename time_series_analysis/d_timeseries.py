...
d_df = pd.read_sql(query, conn)
print(d_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
d_dates = []
for date in d_df['data_date']:
    d_dates.append(datetime.strptime(date, '%Y-%m-%d'))

d_df['data_date'] = d_dates
symbol = d_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(d_dates, d_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(d_df['adjusted_close']), max(d_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
d_df = d_df.drop(['symbol'], axis=1)
d_df['data_date'] = pd.to_datetime(d_df['data_date'])
d_df.set_index('data_date', inplace=True)
d_df.index = pd.DatetimeIndex(d_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = d_df.head(round(len(d_df)*.75))
test = d_df.tail(round(len(d_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2016, 4, 20)
...
# RMSE for Model (10, 1, 2): 12.228736805372705
# FIT THE MODEL
