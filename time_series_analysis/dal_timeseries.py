...
dal_df = pd.read_sql(query, conn)
print(dal_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
dal_dates = []
for date in dal_df['data_date']:
    dal_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dal_df['data_date'] = dal_dates
symbol = dal_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dal_dates, dal_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dal_df['adjusted_close']), max(dal_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dal_df = dal_df.drop(['symbol'], axis=1)
dal_df['data_date'] = pd.to_datetime(dal_df['data_date'])
dal_df.set_index('data_date', inplace=True)
dal_df.index = pd.DatetimeIndex(dal_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dal_df.head(round(len(dal_df)*.75))
test = dal_df.tail(round(len(dal_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 0, 0): 3.5302259499534823
# FIT THE MODEL
...
model = ARIMA(dal_df, order=(6, 0, 0))
model_fit = model.fit()
print(model_fit.summary())
