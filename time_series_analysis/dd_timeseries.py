...
dd_df = pd.read_sql(query, conn)
print(dd_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
dd_dates = []
for date in dd_df['data_date']:
    dd_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dd_df['data_date'] = dd_dates
symbol = dd_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dd_dates, dd_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dd_df['adjusted_close']), max(dd_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dd_df = dd_df.drop(['symbol'], axis=1)
dd_df['data_date'] = pd.to_datetime(dd_df['data_date'])
dd_df.set_index('data_date', inplace=True)
dd_df.index = pd.DatetimeIndex(dd_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dd_df.head(round(len(dd_df)*.75))
test = dd_df.tail(round(len(dd_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 0, 0): 7.384029418761223
# FIT THE MODEL
...
model = ARIMA(dd_df, order=(10, 0, 0))
model_fit = model.fit()
print(model_fit.summary())
