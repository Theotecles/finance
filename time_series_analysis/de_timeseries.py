...
de_df = pd.read_sql(query, conn)
print(de_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
de_dates = []
for date in de_df['data_date']:
    de_dates.append(datetime.strptime(date, '%Y-%m-%d'))

de_df['data_date'] = de_dates
symbol = de_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(de_dates, de_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(de_df['adjusted_close']), max(de_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
de_df = de_df.drop(['symbol'], axis=1)
de_df['data_date'] = pd.to_datetime(de_df['data_date'])
de_df.set_index('data_date', inplace=True)
de_df.index = pd.DatetimeIndex(de_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = de_df.head(round(len(de_df)*.75))
test = de_df.tail(round(len(de_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 0, 2): 18.432542224915316
# FIT THE MODEL
...
model = ARIMA(de_df, order=(10, 0, 2))
model_fit = model.fit()
print(model_fit.summary())
