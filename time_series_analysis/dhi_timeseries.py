...
dhi_df = pd.read_sql(query, conn)
print(dhi_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
dhi_dates = []
for date in dhi_df['data_date']:
    dhi_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dhi_df['data_date'] = dhi_dates
symbol = dhi_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dhi_dates, dhi_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dhi_df['adjusted_close']), max(dhi_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dhi_df = dhi_df.drop(['symbol'], axis=1)
dhi_df['data_date'] = pd.to_datetime(dhi_df['data_date'])
dhi_df.set_index('data_date', inplace=True)
dhi_df.index = pd.DatetimeIndex(dhi_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dhi_df.head(round(len(dhi_df)*.75))
test = dhi_df.tail(round(len(dhi_df)*.25))
print(test.head())
print(test.tail())
...
model = ARIMA(dhi_df, order=(2, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
