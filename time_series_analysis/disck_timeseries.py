...
disck_df = pd.read_sql(query, conn)
print(disck_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
disck_dates = []
for date in disck_df['data_date']:
    disck_dates.append(datetime.strptime(date, '%Y-%m-%d'))

disck_df['data_date'] = disck_dates
symbol = disck_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(disck_dates, disck_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(disck_df['adjusted_close']), max(disck_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
disck_df = disck_df.drop(['symbol'], axis=1)
disck_df['data_date'] = pd.to_datetime(disck_df['data_date'])
disck_df.set_index('data_date', inplace=True)
disck_df.index = pd.DatetimeIndex(disck_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = disck_df.head(round(len(disck_df)*.75))
test = disck_df.tail(round(len(disck_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 0, 0): 8.433161115117592
# FIT THE MODEL
...
model = ARIMA(disck_df, order=(0, 0, 0))
model_fit = model.fit()
print(model_fit.summary())
