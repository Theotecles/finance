...
dish_df = pd.read_sql(query, conn)
print(dish_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
dish_dates = []
for date in dish_df['data_date']:
    dish_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dish_df['data_date'] = dish_dates
symbol = dish_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dish_dates, dish_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dish_df['adjusted_close']), max(dish_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dish_df = dish_df.drop(['symbol'], axis=1)
dish_df['data_date'] = pd.to_datetime(dish_df['data_date'])
dish_df.set_index('data_date', inplace=True)
dish_df.index = pd.DatetimeIndex(dish_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dish_df.head(round(len(dish_df)*.75))
test = dish_df.tail(round(len(dish_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (4, 1, 2): 7.155525537919223
# FIT THE MODEL
...
model = ARIMA(dish_df, order=(4, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
