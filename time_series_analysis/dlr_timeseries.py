...
dlr_df = pd.read_sql(query, conn)
print(dlr_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
dlr_dates = []
for date in dlr_df['data_date']:
    dlr_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dlr_df['data_date'] = dlr_dates
symbol = dlr_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dlr_dates, dlr_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dlr_df['adjusted_close']), max(dlr_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dlr_df = dlr_df.drop(['symbol'], axis=1)
dlr_df['data_date'] = pd.to_datetime(dlr_df['data_date'])
dlr_df.set_index('data_date', inplace=True)
dlr_df.index = pd.DatetimeIndex(dlr_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dlr_df.head(round(len(dlr_df)*.75))
test = dlr_df.tail(round(len(dlr_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 1): 9.855570224605293
# FIT THE MODEL
...
model = ARIMA(dlr_df, order=(6, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
