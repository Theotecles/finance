...
dltr_df = pd.read_sql(query, conn)
print(dltr_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
dltr_dates = []
for date in dltr_df['data_date']:
    dltr_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dltr_df['data_date'] = dltr_dates
symbol = dltr_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dltr_dates, dltr_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dltr_df['adjusted_close']), max(dltr_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dltr_df = dltr_df.drop(['symbol'], axis=1)
dltr_df['data_date'] = pd.to_datetime(dltr_df['data_date'])
dltr_df.set_index('data_date', inplace=True)
dltr_df.index = pd.DatetimeIndex(dltr_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dltr_df.head(round(len(dltr_df)*.75))
test = dltr_df.tail(round(len(dltr_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 0, 0): 13.696255899583983
# FIT THE MODEL
...
model = ARIMA(dltr_df, order=(0, 0, 0))
model_fit = model.fit()
print(model_fit.summary())
