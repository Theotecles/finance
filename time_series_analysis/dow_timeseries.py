...
dow_dates = []
for date in dow_df['data_date']:
    dow_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dow_df['data_date'] = dow_dates
symbol = dow_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dow_dates, dow_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dow_df['adjusted_close']), max(dow_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dow_df = dow_df.drop(['symbol'], axis=1)
dow_df['data_date'] = pd.to_datetime(dow_df['data_date'])
dow_df.set_index('data_date', inplace=True)
dow_df.index = pd.DatetimeIndex(dow_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dow_df.head(round(len(dow_df)*.75))
test = dow_df.tail(round(len(dow_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 2, 23)
...
# RMSE for Model (10, 1, 2): 3.432593993948679
# FIT THE MODEL
...
model = ARIMA(dow_df, order=(10, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
