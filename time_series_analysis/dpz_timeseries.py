...
dpz_df = pd.read_sql(query, conn)
print(dpz_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
dpz_dates = []
for date in dpz_df['data_date']:
    dpz_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dpz_df['data_date'] = dpz_dates
symbol = dpz_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dpz_dates, dpz_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dpz_df['adjusted_close']), max(dpz_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dpz_df = dpz_df.drop(['symbol'], axis=1)
dpz_df['data_date'] = pd.to_datetime(dpz_df['data_date'])
dpz_df.set_index('data_date', inplace=True)
dpz_df.index = pd.DatetimeIndex(dpz_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dpz_df.head(round(len(dpz_df)*.75))
test = dpz_df.tail(round(len(dpz_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 2, 2): 102.38626509430013
# FIT THE MODEL
...
model = ARIMA(dpz_df, order=(10, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
