...
dre_df = pd.read_sql(query, conn)
print(dre_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
dre_dates = []
for date in dre_df['data_date']:
    dre_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dre_df['data_date'] = dre_dates
symbol = dre_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dre_dates, dre_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dre_df['adjusted_close']), max(dre_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dre_df = dre_df.drop(['symbol'], axis=1)
dre_df['data_date'] = pd.to_datetime(dre_df['data_date'])
dre_df.set_index('data_date', inplace=True)
dre_df.index = pd.DatetimeIndex(dre_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dre_df.head(round(len(dre_df)*.75))
test = dre_df.tail(round(len(dre_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 2, 1): 7.634518259093526
# FIT THE MODEL
...
model = ARIMA(dre_df, order=(6, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
