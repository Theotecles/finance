...
duk_df = pd.read_sql(query, conn)
print(duk_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
duk_dates = []
for date in duk_df['data_date']:
    duk_dates.append(datetime.strptime(date, '%Y-%m-%d'))

duk_df['data_date'] = duk_dates
symbol = duk_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(duk_dates, duk_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(duk_df['adjusted_close']), max(duk_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
duk_df = duk_df.drop(['symbol'], axis=1)
duk_df['data_date'] = pd.to_datetime(duk_df['data_date'])
duk_df.set_index('data_date', inplace=True)
duk_df.index = pd.DatetimeIndex(duk_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = duk_df.head(round(len(duk_df)*.75))
test = duk_df.tail(round(len(duk_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 1): 16.964989988165495
# FIT THE MODEL
...
model = ARIMA(duk_df, order=(6, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
