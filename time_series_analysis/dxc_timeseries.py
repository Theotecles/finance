...
query = '''
    ...
    WHERE symbol = 'DXC'
    AND data_date >= '2018-06-01'
    ORDER BY data_date ASC
    '''

# PULL IN THE DATA USING PANDAS
dxc_df = pd.read_sql(query, conn)
print(dxc_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
dxc_dates = []
for date in dxc_df['data_date']:
    dxc_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dxc_df['data_date'] = dxc_dates
symbol = dxc_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dxc_dates, dxc_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dxc_df['adjusted_close']), max(dxc_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dxc_df = dxc_df.drop(['symbol'], axis=1)
dxc_df['data_date'] = pd.to_datetime(dxc_df['data_date'])
dxc_df.set_index('data_date', inplace=True)
dxc_df.index = pd.DatetimeIndex(dxc_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dxc_df.head(round(len(dxc_df)*.75))
test = dxc_df.tail(round(len(dxc_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2020, 12, 7)
...
# RMSE for Model (2, 0, 0): 4.630610157837796
# FIT THE MODEL
...
model = ARIMA(dxc_df, order=(2, 0, 0))
model_fit = model.fit()
print(model_fit.summary())
