...
dxcm_df = pd.read_sql(query, conn)
print(dxcm_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
dxcm_dates = []
for date in dxcm_df['data_date']:
    dxcm_dates.append(datetime.strptime(date, '%Y-%m-%d'))

dxcm_df['data_date'] = dxcm_dates
symbol = dxcm_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(dxcm_dates, dxcm_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(dxcm_df['adjusted_close']), max(dxcm_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
dxcm_df = dxcm_df.drop(['symbol'], axis=1)
dxcm_df['data_date'] = pd.to_datetime(dxcm_df['data_date'])
dxcm_df.set_index('data_date', inplace=True)
dxcm_df.index = pd.DatetimeIndex(dxcm_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = dxcm_df.head(round(len(dxcm_df)*.75))
test = dxcm_df.tail(round(len(dxcm_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 1, 2): 80.9901856230966
# FIT THE MODEL
...
model = ARIMA(dxcm_df, order=(0, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
