...
ea_df = pd.read_sql(query, conn)
print(ea_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
ea_dates = []
for date in ea_df['data_date']:
    ea_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ea_df['data_date'] = ea_dates
symbol = ea_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ea_dates, ea_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ea_df['adjusted_close']), max(ea_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ea_df = ea_df.drop(['symbol'], axis=1)
ea_df['data_date'] = pd.to_datetime(ea_df['data_date'])
ea_df.set_index('data_date', inplace=True)
ea_df.index = pd.DatetimeIndex(ea_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ea_df.head(round(len(ea_df)*.75))
test = ea_df.tail(round(len(ea_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (8, 1, 0): 4.936023494571757
# FIT THE MODEL
