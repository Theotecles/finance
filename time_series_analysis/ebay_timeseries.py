...
ebay_df = pd.read_sql(query, conn)
print(ebay_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
ebay_dates = []
for date in ebay_df['data_date']:
    ebay_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ebay_df['data_date'] = ebay_dates
symbol = ebay_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ebay_dates, ebay_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ebay_df['adjusted_close']), max(ebay_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ebay_df = ebay_df.drop(['symbol'], axis=1)
ebay_df['data_date'] = pd.to_datetime(ebay_df['data_date'])
ebay_df.set_index('data_date', inplace=True)
ebay_df.index = pd.DatetimeIndex(ebay_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ebay_df.head(round(len(ebay_df)*.75))
test = ebay_df.tail(round(len(ebay_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 2, 2): 4.064768746172245
# FIT THE MODEL
...
model = ARIMA(ebay_df, order=(10, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
