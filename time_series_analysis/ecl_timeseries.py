...
ecl_df = pd.read_sql(query, conn)
print(ecl_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
ecl_dates = []
for date in ecl_df['data_date']:
    ecl_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ecl_df['data_date'] = ecl_dates
symbol = ecl_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ecl_dates, ecl_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ecl_df['adjusted_close']), max(ecl_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ecl_df = ecl_df.drop(['symbol'], axis=1)
ecl_df['data_date'] = pd.to_datetime(ecl_df['data_date'])
ecl_df.set_index('data_date', inplace=True)
ecl_df.index = pd.DatetimeIndex(ecl_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ecl_df.head(round(len(ecl_df)*.75))
test = ecl_df.tail(round(len(ecl_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 1): 20.69127962103334
# FIT THE MODEL
...
model = ARIMA(ecl_df, order=(6, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
