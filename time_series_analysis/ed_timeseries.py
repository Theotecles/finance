...
ed_dates = []
for date in ed_df['data_date']:
    ed_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ed_df['data_date'] = ed_dates
symbol = ed_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ed_dates, ed_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ed_df['adjusted_close']), max(ed_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ed_df = ed_df.drop(['symbol'], axis=1)
ed_df['data_date'] = pd.to_datetime(ed_df['data_date'])
ed_df.set_index('data_date', inplace=True)
ed_df.index = pd.DatetimeIndex(ed_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ed_df.head(round(len(ed_df)*.75))
test = ed_df.tail(round(len(ed_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 0, 2): 1.8457076064026368
# FIT THE MODEL
