...
efx_df = pd.read_sql(query, conn)
print(efx_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
efx_dates = []
for date in efx_df['data_date']:
    efx_dates.append(datetime.strptime(date, '%Y-%m-%d'))

efx_df['data_date'] = efx_dates
symbol = efx_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(efx_dates, efx_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(efx_df['adjusted_close']), max(efx_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
efx_df = efx_df.drop(['symbol'], axis=1)
efx_df['data_date'] = pd.to_datetime(efx_df['data_date'])
efx_df.set_index('data_date', inplace=True)
efx_df.index = pd.DatetimeIndex(efx_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = efx_df.head(round(len(efx_df)*.75))
test = efx_df.tail(round(len(efx_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 2, 1): 45.28852304546285
# FIT THE MODEL
...
model = ARIMA(efx_df, order=(10, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
