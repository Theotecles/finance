...
eix_df = pd.read_sql(query, conn)
print(eix_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
eix_dates = []
for date in eix_df['data_date']:
    eix_dates.append(datetime.strptime(date, '%Y-%m-%d'))

eix_df['data_date'] = eix_dates
symbol = eix_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(eix_dates, eix_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(eix_df['adjusted_close']), max(eix_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
eix_df = eix_df.drop(['symbol'], axis=1)
eix_df['data_date'] = pd.to_datetime(eix_df['data_date'])
eix_df.set_index('data_date', inplace=True)
eix_df.index = pd.DatetimeIndex(eix_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = eix_df.head(round(len(eix_df)*.75))
test = eix_df.tail(round(len(eix_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 2, 2): 3.9799128043559477
# FIT THE MODEL
model = ARIMA(train, order=(6, 2, 2))
model_fit = model.fit()
...
model = ARIMA(eix_df, order=(6, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
