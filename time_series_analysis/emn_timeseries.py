...
emn_df = pd.read_sql(query, conn)
print(emn_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
emn_dates = []
for date in emn_df['data_date']:
    emn_dates.append(datetime.strptime(date, '%Y-%m-%d'))

emn_df['data_date'] = emn_dates
symbol = emn_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(emn_dates, emn_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(emn_df['adjusted_close']), max(emn_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
emn_df = emn_df.drop(['symbol'], axis=1)
emn_df['data_date'] = pd.to_datetime(emn_df['data_date'])
emn_df.set_index('data_date', inplace=True)
emn_df.index = pd.DatetimeIndex(emn_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = emn_df.head(round(len(emn_df)*.75))
test = emn_df.tail(round(len(emn_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (4, 1, 0): 7.782952808052798
# FIT THE MODEL
...
model = ARIMA(emn_df, order=(4, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
