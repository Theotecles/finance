...
emr_df = pd.read_sql(query, conn)
print(emr_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
emr_dates = []
for date in emr_df['data_date']:
    emr_dates.append(datetime.strptime(date, '%Y-%m-%d'))

emr_df['data_date'] = emr_dates
symbol = emr_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(emr_dates, emr_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(emr_df['adjusted_close']), max(emr_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
emr_df = emr_df.drop(['symbol'], axis=1)
emr_df['data_date'] = pd.to_datetime(emr_df['data_date'])
emr_df.set_index('data_date', inplace=True)
emr_df.index = pd.DatetimeIndex(emr_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = emr_df.head(round(len(emr_df)*.75))
test = emr_df.tail(round(len(emr_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 5, 28)
...
# RMSE for Model (0, 1, 0): 4.700597900114062
# FIT THE MODEL
...
model = ARIMA(emr_df, order=(0, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
