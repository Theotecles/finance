...
enph_df = pd.read_sql(query, conn)
print(enph_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
enph_dates = []
for date in enph_df['data_date']:
    enph_dates.append(datetime.strptime(date, '%Y-%m-%d'))

enph_df['data_date'] = enph_dates
symbol = enph_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(enph_dates, enph_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(enph_df['adjusted_close']), max(enph_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
enph_df = enph_df.drop(['symbol'], axis=1)
enph_df['data_date'] = pd.to_datetime(enph_df['data_date'])
enph_df.set_index('data_date', inplace=True)
enph_df.index = pd.DatetimeIndex(enph_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = enph_df.head(round(len(enph_df)*.75))
test = enph_df.tail(round(len(enph_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 0): 35.63950228742766
# FIT THE MODEL
...
model = ARIMA(enph_df, order=(6, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
