...
eog_df = pd.read_sql(query, conn)
print(eog_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
eog_dates = []
for date in eog_df['data_date']:
    eog_dates.append(datetime.strptime(date, '%Y-%m-%d'))

eog_df['data_date'] = eog_dates
symbol = eog_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(eog_dates, eog_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(eog_df['adjusted_close']), max(eog_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
eog_df = eog_df.drop(['symbol'], axis=1)
eog_df['data_date'] = pd.to_datetime(eog_df['data_date'])
eog_df.set_index('data_date', inplace=True)
eog_df.index = pd.DatetimeIndex(eog_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = eog_df.head(round(len(eog_df)*.75))
test = eog_df.tail(round(len(eog_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 1, 2): 20.730123187225715
# FIT THE MODEL
...
model = ARIMA(eog_df, order=(10, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
