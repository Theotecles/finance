...
eqix_df = pd.read_sql(query, conn)
print(eqix_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
eqix_dates = []
for date in eqix_df['data_date']:
    eqix_dates.append(datetime.strptime(date, '%Y-%m-%d'))

eqix_df['data_date'] = eqix_dates
symbol = eqix_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(eqix_dates, eqix_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(eqix_df['adjusted_close']), max(eqix_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
eqix_df = eqix_df.drop(['symbol'], axis=1)
eqix_df['data_date'] = pd.to_datetime(eqix_df['data_date'])
eqix_df.set_index('data_date', inplace=True)
eqix_df.index = pd.DatetimeIndex(eqix_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = eqix_df.head(round(len(eqix_df)*.75))
test = eqix_df.tail(round(len(eqix_df)*.25))
print(test.head())
print(test.tail())
...
# RRMSE for Model (8, 2, 1): 86.26826106563372
# FIT THE MODEL
...
model = ARIMA(eqix_df, order=(8, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
