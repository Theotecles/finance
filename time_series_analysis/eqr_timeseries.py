...
eqr_df = pd.read_sql(query, conn)
print(eqr_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
eqr_dates = []
for date in eqr_df['data_date']:
    eqr_dates.append(datetime.strptime(date, '%Y-%m-%d'))

eqr_df['data_date'] = eqr_dates
symbol = eqr_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(eqr_dates, eqr_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(eqr_df['adjusted_close']), max(eqr_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
eqr_df = eqr_df.drop(['symbol'], axis=1)
eqr_df['data_date'] = pd.to_datetime(eqr_df['data_date'])
eqr_df.set_index('data_date', inplace=True)
eqr_df.index = pd.DatetimeIndex(eqr_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = eqr_df.head(round(len(eqr_df)*.75))
test = eqr_df.tail(round(len(eqr_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 7, 14)
...
# RMSE for Model (6, 1, 0): 1.953726960520353
# FIT THE MODEL
...
model = ARIMA(eqr_df, order=(8, 2, 1))
model_fit = model.fit()
print(model_fit.summary())
