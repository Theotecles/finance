...
ess_df = pd.read_sql(query, conn)
print(ess_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
ess_dates = []
for date in ess_df['data_date']:
    ess_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ess_df['data_date'] = ess_dates
symbol = ess_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ess_dates, ess_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ess_df['adjusted_close']), max(ess_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ess_df = ess_df.drop(['symbol'], axis=1)
ess_df['data_date'] = pd.to_datetime(ess_df['data_date'])
ess_df.set_index('data_date', inplace=True)
ess_df.index = pd.DatetimeIndex(ess_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ess_df.head(round(len(ess_df)*.75))
test = ess_df.tail(round(len(ess_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (4, 1, 2): 5.84929532066677
# FIT THE MODEL
...
model = ARIMA(ess_df, order=(4, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
