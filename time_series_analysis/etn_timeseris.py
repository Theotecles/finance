...
etn_df = pd.read_sql(query, conn)
print(etn_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
etn_dates = []
for date in etn_df['data_date']:
    etn_dates.append(datetime.strptime(date, '%Y-%m-%d'))

etn_df['data_date'] = etn_dates
symbol = etn_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(etn_dates, etn_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(etn_df['adjusted_close']), max(etn_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
etn_df = etn_df.drop(['symbol'], axis=1)
etn_df['data_date'] = pd.to_datetime(etn_df['data_date'])
etn_df.set_index('data_date', inplace=True)
etn_df.index = pd.DatetimeIndex(etn_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = etn_df.head(round(len(etn_df)*.75))
test = etn_df.tail(round(len(etn_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 1, 0): 12.28568333779867
# FIT THE MODEL
...
model = ARIMA(etn_df, order=(10, 1, 0))
model_fit = model.fit()
print(model_fit.summary())
