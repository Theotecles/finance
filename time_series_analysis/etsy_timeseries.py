...
etsy_df = pd.read_sql(query, conn)
print(etsy_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
etsy_dates = []
for date in etsy_df['data_date']:
    etsy_dates.append(datetime.strptime(date, '%Y-%m-%d'))

etsy_df['data_date'] = etsy_dates
symbol = etsy_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(etsy_dates, etsy_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(etsy_df['adjusted_close']), max(etsy_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
etsy_df = etsy_df.drop(['symbol'], axis=1)
etsy_df['data_date'] = pd.to_datetime(etsy_df['data_date'])
etsy_df.set_index('data_date', inplace=True)
etsy_df.index = pd.DatetimeIndex(etsy_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = etsy_df.head(round(len(etsy_df)*.75))
test = etsy_df.tail(round(len(etsy_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (8, 2, 2): 30.9613297211779
# FIT THE MODEL
...
model = ARIMA(etsy_df, order=(8, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
