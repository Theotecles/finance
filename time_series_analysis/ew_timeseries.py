...
ew_df = pd.read_sql(query, conn)
print(ew_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
ew_dates = []
for date in ew_df['data_date']:
    ew_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ew_df['data_date'] = ew_dates
symbol = ew_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ew_dates, ew_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ew_df['adjusted_close']), max(ew_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ew_df = ew_df.drop(['symbol'], axis=1)
ew_df['data_date'] = pd.to_datetime(ew_df['data_date'])
ew_df.set_index('data_date', inplace=True)
ew_df.index = pd.DatetimeIndex(ew_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ew_df.head(round(len(ew_df)*.75))
test = ew_df.tail(round(len(ew_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (4, 2, 1): 14.466448500866184
# FIT THE MODEL
