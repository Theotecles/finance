...
exc_df = pd.read_sql(query, conn)
print(exc_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
exc_dates = []
for date in exc_df['data_date']:
    exc_dates.append(datetime.strptime(date, '%Y-%m-%d'))

exc_df['data_date'] = exc_dates
symbol = exc_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(exc_dates, exc_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(exc_df['adjusted_close']), max(exc_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
exc_df = exc_df.drop(['symbol'], axis=1)
exc_df['data_date'] = pd.to_datetime(exc_df['data_date'])
exc_df.set_index('data_date', inplace=True)
exc_df.index = pd.DatetimeIndex(exc_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = exc_df.head(round(len(exc_df)*.75))
test = exc_df.tail(round(len(exc_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 2, 2): 4.061537796550229
# FIT THE MODEL
...
model = ARIMA(exc_df, order=(10, 2, 2))
model_fit = model.fit()
print(model_fit.summary())
