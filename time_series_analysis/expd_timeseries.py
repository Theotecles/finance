...
expd_df = pd.read_sql(query, conn)
print(expd_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
expd_dates = []
for date in expd_df['data_date']:
    expd_dates.append(datetime.strptime(date, '%Y-%m-%d'))

expd_df['data_date'] = expd_dates
symbol = expd_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(expd_dates, expd_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(expd_df['adjusted_close']), max(expd_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
expd_df = expd_df.drop(['symbol'], axis=1)
expd_df['data_date'] = pd.to_datetime(expd_df['data_date'])
expd_df.set_index('data_date', inplace=True)
expd_df.index = pd.DatetimeIndex(expd_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = expd_df.head(round(len(expd_df)*.75))
test = expd_df.tail(round(len(expd_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (8, 2, 0): 12.744362922436913
# FIT THE MODEL
...
model = ARIMA(expd_df, order=(8, 2, 0))
model_fit = model.fit()
print(model_fit.summary())
