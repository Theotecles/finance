...
expe_df = pd.read_sql(query, conn)
print(expe_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
expe_dates = []
for date in expe_df['data_date']:
    expe_dates.append(datetime.strptime(date, '%Y-%m-%d'))

expe_df['data_date'] = expe_dates
symbol = expe_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(expe_dates, expe_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(expe_df['adjusted_close']), max(expe_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
expe_df = expe_df.drop(['symbol'], axis=1)
expe_df['data_date'] = pd.to_datetime(expe_df['data_date'])
expe_df.set_index('data_date', inplace=True)
expe_df.index = pd.DatetimeIndex(expe_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = expe_df.head(round(len(expe_df)*.75))
test = expe_df.tail(round(len(expe_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (10, 0, 2): 10.572560982085708
# FIT THE MODEL
...
model = ARIMA(expe_df, order=(10, 0, 2))
model_fit = model.fit()
print(model_fit.summary())
