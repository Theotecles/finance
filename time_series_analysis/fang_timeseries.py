...
fang_df = pd.read_sql(query, conn)
print(fang_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
fang_dates = []
for date in fang_df['data_date']:
    fang_dates.append(datetime.strptime(date, '%Y-%m-%d'))

fang_df['data_date'] = fang_dates
symbol = fang_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(fang_dates, fang_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(fang_df['adjusted_close']), max(fang_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
fang_df = fang_df.drop(['symbol'], axis=1)
fang_df['data_date'] = pd.to_datetime(fang_df['data_date'])
fang_df.set_index('data_date', inplace=True)
fang_df.index = pd.DatetimeIndex(fang_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = fang_df.head(round(len(fang_df)*.75))
test = fang_df.tail(round(len(fang_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2021, 5, 27)
...
# RMSE for Model (4, 1, 1): 11.917958872875278
# FIT THE MODEL
...
model = ARIMA(fang_df, order=(4, 1, 1))
model_fit = model.fit()
print(model_fit.summary())
