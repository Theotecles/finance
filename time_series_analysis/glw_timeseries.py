...
glw_df = pd.read_sql(query, conn)
print(glw_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
glw_dates = []
for date in glw_df['data_date']:
    glw_dates.append(datetime.strptime(date, '%Y-%m-%d'))

glw_df['data_date'] = glw_dates
symbol = glw_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(glw_dates, glw_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(glw_df['adjusted_close']), max(glw_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
glw_df = glw_df.drop(['symbol'], axis=1)
glw_df['data_date'] = pd.to_datetime(glw_df['data_date'])
glw_df.set_index('data_date', inplace=True)
glw_df.index = pd.DatetimeIndex(glw_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = glw_df.head(round(len(glw_df)*.75))
test = glw_df.tail(round(len(glw_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 1, 2): 6.245137776204981
# FIT THE MODEL
...
model = ARIMA(glw_df, order=(6, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
