...
ko_df = pd.read_sql(query, conn)
print(ko_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
ko_dates = []
for date in ko_df['data_date']:
    ko_dates.append(datetime.strptime(date, '%Y-%m-%d'))

ko_df['data_date'] = ko_dates
symbol = ko_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(ko_dates, ko_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(ko_df['adjusted_close']), max(ko_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
ko_df = ko_df.drop(['symbol'], axis=1)
ko_df['data_date'] = pd.to_datetime(ko_df['data_date'])
ko_df.set_index('data_date', inplace=True)
ko_df.index = pd.DatetimeIndex(ko_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = ko_df.head(round(len(ko_df)*.75))
test = ko_df.tail(round(len(ko_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (0, 1, 0): 7.775226124913174
# FIT THE MODEL
