...
lnt_df = pd.read_sql(query, conn)
print(lnt_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
lnt_dates = []
for date in lnt_df['data_date']:
    lnt_dates.append(datetime.strptime(date, '%Y-%m-%d'))

lnt_df['data_date'] = lnt_dates

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(lnt_dates, lnt_df['adjusted_close'], c='red', alpha=0.6)

# FORMAT PLOT
ax.set_title(f"Daily Adjusted Close Data \n({lnt_df['symbol'][1]})", fontsize=24)
ax.set_xlabel('', fontsize=16)
fig.autofmt_xdate()
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(lnt_df['adjusted_close']), max(lnt_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
lnt_df = lnt_df.drop(['symbol'], axis=1)
lnt_df['data_date'] = pd.to_datetime(lnt_df['data_date'])
lnt_df.set_index('data_date', inplace=True)
lnt_df.index = pd.DatetimeIndex(lnt_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = lnt_df.head(round(len(lnt_df)*.75))
test = lnt_df.tail(round(len(lnt_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (4, 1, 2): 9.207218820922332
# FIT THE MODEL
...
model = ARIMA(lnt_df, order=(4, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
