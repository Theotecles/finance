...
query = '''
    ...
    WHERE symbol = 'MMM'
    AND data_date >= '2021-08-01'
    ORDER BY data_date ASC
    '''

# PULL IN THE DATA USING PANDAS
mmm_df = pd.read_sql(query, conn)
print(mmm_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
mmm_dates = []
for date in mmm_df['data_date']:
    mmm_dates.append(datetime.strptime(date, '%Y-%m-%d'))

mmm_df['data_date'] = mmm_dates

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(mmm_dates, mmm_df['adjusted_close'], c='red', alpha=0.6)

# FORMAT PLOT
ax.set_title(f"Daily Adjusted Close Data \n({mmm_df['symbol'][1]})", fontsize=24)
ax.set_xlabel('', fontsize=16)
fig.autofmt_xdate()
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(mmm_df['adjusted_close']), max(mmm_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
mmm_df = mmm_df.drop(['symbol'], axis=1)
mmm_df['data_date'] = pd.to_datetime(mmm_df['data_date'])
mmm_df.set_index('data_date', inplace=True)
mmm_df.index = pd.DatetimeIndex(mmm_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = mmm_df.head(round(len(mmm_df)*.75))
test = mmm_df.tail(round(len(mmm_df)*.25))
print(test.head())
print(test.tail())
...
# 0, 1, 0 HAS BEST AIC WITH 116.302
# RMSE for Model (2, 1, 2): 2.8092112470958233
...
model = ARIMA(mmm_df, order=(2, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
...
ax.set_ylabel("Price USD", fontsize=10)
ax.tick_params(axis='both', which='major', labelsize=10)
plt.ylim(min(projections), max(mmm_df['adjusted_close']))
