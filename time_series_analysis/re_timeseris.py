...
re_df = pd.read_sql(query, conn)
print(re_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
re_dates = []
for date in re_df['data_date']:
    re_dates.append(datetime.strptime(date, '%Y-%m-%d'))

re_df['data_date'] = re_dates
symbol = re_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(re_dates, re_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(re_df['adjusted_close']), max(re_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
re_df = re_df.drop(['symbol'], axis=1)
re_df['data_date'] = pd.to_datetime(re_df['data_date'])
re_df.set_index('data_date', inplace=True)
re_df.index = pd.DatetimeIndex(re_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = re_df.head(round(len(re_df)*.75))
test = re_df.tail(round(len(re_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (1, 2, 2): 23.59248191350328
# FIT THE MODEL
