...
schw_df = pd.read_sql(query, conn)
print(schw_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
schw_dates = []
for date in schw_df['data_date']:
    schw_dates.append(datetime.strptime(date, '%Y-%m-%d'))

schw_df['data_date'] = schw_dates
symbol = schw_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(schw_dates, schw_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(schw_df['adjusted_close']), max(schw_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
schw_df = schw_df.drop(['symbol'], axis=1)
schw_df['data_date'] = pd.to_datetime(schw_df['data_date'])
schw_df.set_index('data_date', inplace=True)
schw_df.index = pd.DatetimeIndex(schw_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = schw_df.head(round(len(schw_df)*.75))
test = schw_df.tail(round(len(schw_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (8, 1, 2): 2.596704224638752
# FIT THE MODEL
...
model = ARIMA(schw_df, order=(8, 1, 2))
model_fit = model.fit()
print(model_fit.summary())
