...
t_df = pd.read_sql(query, conn)
print(t_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
t_dates = []
for date in t_df['data_date']:
    t_dates.append(datetime.strptime(date, '%Y-%m-%d'))

t_df['data_date'] = t_dates
symbol = t_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(t_dates, t_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(t_df['adjusted_close']), max(t_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
t_df = t_df.drop(['symbol'], axis=1)
t_df['data_date'] = pd.to_datetime(t_df['data_date'])
t_df.set_index('data_date', inplace=True)
t_df.index = pd.DatetimeIndex(t_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = t_df.head(round(len(t_df)*.75))
test = t_df.tail(round(len(t_df)*.25))
print(test.head())
print(test.tail())
...
start_index = datetime(2020, 7, 28)
...
# RMSE for Model (6, 1, 0): 1.3371900190298514
# FIT THE MODEL
