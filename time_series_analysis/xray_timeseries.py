...
xray_df = pd.read_sql(query, conn)
print(xray_df.head())

# PUT THE DATES INTO THE CORRECT FORMAT FOR MATPLOT
xray_dates = []
for date in xray_df['data_date']:
    xray_dates.append(datetime.strptime(date, '%Y-%m-%d'))

xray_df['data_date'] = xray_dates
symbol = xray_df['symbol'][1]

# PLOT THE DATA
plt.style.use('seaborn')
fig, ax = plt.subplots()
ax.plot(xray_dates, xray_df['adjusted_close'], c='red', alpha=0.6)
...
plt.ylim(min(xray_df['adjusted_close']), max(xray_df['adjusted_close']))

plt.show()

# CONVERT DATAFRAME TO A DATETIME INDEX
xray_df = xray_df.drop(['symbol'], axis=1)
xray_df['data_date'] = pd.to_datetime(xray_df['data_date'])
xray_df.set_index('data_date', inplace=True)
xray_df.index = pd.DatetimeIndex(xray_df.index).to_period('D')

# EVALUATE COMBINATIONS OF P, D AND Q VALUES FOR AN ARIMA MODEL
# SPLIT THE DATA INTO TRAINING AND TEST SET
train = xray_df.head(round(len(xray_df)*.75))
test = xray_df.tail(round(len(xray_df)*.25))
print(test.head())
print(test.tail())
...
# RMSE for Model (6, 0, 2): 2.120822405027295
# FIT THE MODEL
...
model = ARIMA(xray_df, order=(6, 0, 2))
model_fit = model.fit()
print(model_fit.summary())
